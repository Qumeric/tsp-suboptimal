...
double solve_branch_and_bound_STRANGE(double graph[MAX_SIZE][MAX_SIZE], size_t n) {
    vector<vector<double>> matrix(n, vector<double>(n)); // NOT OPTIMAL
    double r = 0;
    vector<double> cs;
    for (int i = 0; i < n; i++) {
        size_t c = (i == 0);
        for (int j = 0; j < n; j++) {
            if (i != j && graph[i][j] < graph[i][c]) {
                c = j;
            }
        }
        r += graph[i][c];
        cs.push_back(graph[i][c]);
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j) matrix[i][j] = graph[i][j] - cs[i];
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        size_t c = (i == 0);
        for (int j = 0; j < n; j++) {
            if (i != j && matrix[j][i] < matrix[c][i]) {
                c = j;
            }
        }
        r += graph[c][i];
        cs.push_back(matrix[c][i]);
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j) matrix[j][i] -= cs[i];
        }
    }
    ...
    cs.clear();
    vector<double> row_sums(n);
    vector<double> col_sums(n);
    ...
    // FIXME do not understand
    pair<size_t, size_t> res = {-1, -1};
    double price = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j && matrix[i][j] == 0) {
                if (row_sums[i] + col_sums[j] > price) {
                    price = row_sums[i] + col_sums[j];
                    res = {i, j};
                }
            }
        }
